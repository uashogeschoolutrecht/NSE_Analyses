...
rwa_bijdrage <- array(NA, dim = c(Nvorm, NHO, length(vec_rwavariabelen)))
rwa_scores   <- array(NA, dim = c(Nvorm, NHO,length(vec_rwavariabelen)))
N            <- array(NA, dim = c(Nvorm, NHO))
for (idx_ho in 1:NHO)
{
  for (idx_vorm in 1:Nvorm)
  {
    # Groepsgroote (N) berekenen
    N[idx_vorm,idx_ho] <-nrow(HUscores_clean[vec_vorm[[idx_vorm]]&vec_HO[[idx_ho]],])
    
    # Als N < 10 dan geen rwa uitvoeren
    if (N[idx_vorm,idx_ho] < 10)
    {
    ...
    else
    {
      ...
      rwa_resultaten <- HUscores_clean[vec_vorm[[idx_vorm]]&vec_HO[[idx_ho]],] %>% 
        rwa(outcome    = NSEvragen[vec_rwauitkomst],
            predictors = NSEvragen[vec_rwavariabelen],
            applysigns = TRUE)
      rwa_bijdrage[idx_vorm,idx_ho,] <- rwa_resultaten$result$Sign.Rescaled.RelWeight #Bijdrage aan de totaalscore
      rwa_scores[idx_vorm,idx_ho,]   <- unlist(lapply(HUscores_clean[vec_vorm[[idx_vorm]]&
                                                                       vec_HO[[idx_ho]],
                                                                     vec_rwavragen[2:9]], 
                                                      mean))
    } #Sluiten rwa loop
