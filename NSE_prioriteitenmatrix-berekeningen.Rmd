...
int_crohos <- length(crohos)
vec_croho <- vector(mode = "list", int_crohos)
for (h in 1:int_crohos)
{
  vec_croho[[h]] <- HUscores_hers$CrohoNaamActueel == crohos[h]
}
vec_croho[[int_crohos+1]] <- rep(TRUE, nrow(HUscores_hers)); 
int_crohos <- length(vec_croho)
...
vec_croho[[int_crohos+1]] <- vec_instituut
int_crohos <- length(vec_croho)
...
rwa_bijdrage <- array(NA, dim = c(Nvorm, int_NHO, int_crohos, length(vec_rwavariabelen)))
rwa_scores   <- array(NA, dim = c(Nvorm, int_NHO, int_crohos, length(vec_rwavariabelen)))
rwa_R2       <- array(NA, dim = c(Nvorm, int_NHO, int_crohos))
N            <- array(NA, dim = c(Nvorm, int_NHO, int_crohos))
...
for (ind_croho in 1:int_crohos)
{
  for (ind_ho in 1:int_NHO)
  {
    for (ind_vorm in 1:Nvorm)
    {
      ...
      if (N[ind_vorm,ind_ho,ind_croho] < 10)
      {
      ...
      else
      {
        ...
        rwa_resultaten <- HUdoorsnede %>%
          rwa(outcome    = str_NSEvragen[vec_rwauitkomst],
              predictors = str_NSEvragen[vec_rwavariabelen],
              applysigns = TRUE)
...
text(rwa_bijdrage[ind_vorm,ind_ho,ind_croho,], #Toevoegen nummer van vraag aan plot
     rwa_scores[ind_vorm,ind_ho,ind_croho,], 
     str_NSEvragen_kort[vec_rwavariabelen], 
     cex=0.7, 
     pos=2, 
     col="blue")
```
  ...
  for (ind_ho in 1:int_NHO)
  {
    for (ind_vorm in 1:Nvorm)
    {
      tmp <- data.frame(Opleidingsvorm3 = NSEvormen[ind_vorm],
                        ...
                        NSEvragen       = str_NSEvragen[vec_rwavariabelen],
...
write.csv(prioriteitenmatrix,file = "~/research-drive/M21033303_DenA (Projectfolder)/DA_Onderzoek/2021nse/data_uit/NSEprioriteitenttabel2022.csv")
```
