...
rwa_bijdrage <- array(NA, dim = c(Nvorm, length(vec_rwavariabelen)))
rwa_scores   <- array(NA, dim = c(Nvorm, length(vec_rwavariabelen)))
N            <- array(NA, dim = c(Nvorm))
for (idx_vorm in 1:Nvorm)
{
  rwa_resultaten <- HUscores_clean[vec_vorm[[idx_vorm]],] %>% 
    rwa(outcome    = NSEvragen[vec_rwauitkomst],
        ...
        applysigns = TRUE)
  rwa_bijdrage[idx_vorm,] <- rwa_resultaten$result$Sign.Rescaled.RelWeight #Bijdrage aan de totaalscore
  rwa_scores[idx_vorm,]   <- unlist(lapply(HUscores_clean[vec_vorm[[idx_vorm]],
                                                          vec_rwavragen[2:9]], mean))
  N[idx_vorm]             <-nrow(HUscores_clean[vec_vorm[[idx_vorm]],])
}
rwa_resultaten
```
