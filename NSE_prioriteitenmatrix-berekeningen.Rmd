...
vec_instituut <- HUscores_hers$CrohoNaamActueel %in% NSEcrohos[c(43:56,80:88)]
vec_croho[[Ncrohos]] <- vec_instituut

#Maak voor elke vorm een logische vector
vec_vorm <- vector(mode = "list", length(int_vorm))
for (i in NSEvormen)
{
...
vec_HO <- vector(mode = "list", length(NHO))
for (j in NSEHOtypes)
{
...
rwa_bijdrage <- array(NA, dim = c(Nvorm, int_NHO, Ncrohos, length(vec_rwavariabelen)))
rwa_scores   <- array(NA, dim = c(Nvorm, int_NHO, Ncrohos, length(vec_rwavariabelen)))
rwa_R2       <- array(NA, dim = c(Nvorm, int_NHO, Ncrohos))
N            <- array(NA, dim = c(Nvorm, int_NHO, Ncrohos))
