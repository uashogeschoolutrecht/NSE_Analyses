...
vec_croho[[Ncrohos+1]] <- rep(TRUE, nrow(nse_benchmark_clean)); Ncrohos <- length(vec_croho)
...
```{r T-test statistiek}
#Bijeen zoeken van wegingsdata
Weging      <- nse_benchmark_clean$Weegfactor
...
for (i_ho in 1:NHO)
{
  ...
  for (i_vorm in 1:Nvorm)
  {
    ...
    for (i_vraag in vec_NSEvragen)
    {
      ...
      N_HU[i_vraag-Ncor, i_vorm, i_ho, i_croho] <- sum(!is.na(nse_benchmark_clean[
        (vec_groepHU & vec_HO[[i_ho]]&vec_vorm[[i_vorm]] & vec_croho[[i_croho]]),i_vraag]))
      N_R6[i_vraag-Ncor, i_vorm, i_ho, i_croho] <- sum(!is.na(nse_benchmark_clean[
        (vec_groepR6 & vec_HO[[i_ho]]&vec_vorm[[i_vorm]] & vec_croho[[i_croho]]),i_vraag]))
      N_AL[i_vraag-Ncor, i_vorm, i_ho, i_croho] <- sum(!is.na(nse_benchmark_clean[
        (vec_groepAL & vec_HO[[i_ho]]&vec_vorm[[i_vorm]] & vec_croho[[i_croho]]),i_vraag]))
      
      #Berekenen gemiddelden per groep (gewogen)
      gem_HU[i_vraag-Ncor,i_vorm,i_ho, i_croho]  <- weighted.mean(nse_benchmark_clean
                                                                  [(vec_groepHU 
                                                                    &vec_HO[[i_ho]]
                                                                    &vec_vorm[[i_vorm]]
                                                                    &vec_croho[[i_croho]])
                                                                  ...
                                                                  Weging[(vec_groepHU
                                                                          ...
                                                                          &vec_croho[[i_croho]])],
                                                                  na.rm = TRUE)
      gem_R6[i_vraag-Ncor,i_vorm,i_ho, i_croho]  <- weighted.mean(nse_benchmark_clean
                                                                  [(vec_groepR6
                                                                    & vec_HO[[i_ho]]
                                                                    &vec_vorm[[i_vorm]]
                                                                    &vec_croho[[i_croho]])
                                                                  ...
                                                                  Weging[(vec_groepR6 
                                                                          ...
                                                                          &vec_croho[[i_croho]])],
                                                                  na.rm = TRUE)
      gem_AL[i_vraag-Ncor,i_vorm,i_ho, i_croho]  <- weighted.mean(nse_benchmark_clean
                                                                  [(vec_groepAL &vec_HO[[i_ho]]
                                                                    &vec_vorm[[i_vorm]]
                                                                    &vec_croho[[i_croho]])
                                                                    ,i_vraag],
                                                                  Weging[(vec_groepAL 
      ...
      {
        #Berekenen p waarden
        p_HUR6[i_vraag-Ncor,i_vorm,i_ho,i_croho]  <- t.test(nse_benchmark_clean
                                                            [(vec_groepHU 
                                                              &vec_HO[[i_ho]]
                                                              &vec_vorm[[i_vorm]]
                                                              &vec_croho[[i_croho]])
        ...
        p_HUAL[i_vraag-Ncor,i_vorm,i_ho,i_croho]  <- t.test(nse_benchmark_clean
                                                            [(vec_groepHU 
                                                              &vec_HO[[i_ho]]
                                                              &vec_vorm[[i_vorm]]
                                                              &vec_croho[[i_croho]])
...
Het niveau waarop gecorrigeerd wordt is de kleinste granulariteit, namelijk: croho-vorm-typeHO combinatie. De argumentatie hiervoor is dat de voornamelijke doelgroep de opleidingsmanagers (croho) zijn; het is dus alsof elke opleidingsmanager (d.w.z. op niveau croho, waarbij vorm en typeHO al vastliggen) zijn eigen significantie-toetsing doet. 
...
plot(p_HUR6_adj[,i_vorm,i_ho])
abline(h = 0.05)
groepering <- ifelse(diff_HUR6[,i_vorm,i_ho]  > 0 & 
                     p_HUR6_adj[,i_vorm,i_ho] < 0.05, 3, 
              ifelse(diff_HUR6[,i_vorm,i_ho]  < 0 & 
                     p_HUR6_adj[,i_vorm,i_ho] < 0.05, 2, 1))
plot(diff_HUR6[,i_vorm,i_ho], col = groepering, pch = 19, 
     xlab = "NSE vraag", 
     ylab = "Gemiddelde verschil (delta) tussen HU en R6")
```

```{r Visualiseren test-statistiek HU <> AL}
plot(p_HUAL_adj[,i_vorm,i_ho])
abline(h = 0.05)
groepering <- ifelse(diff_HUAL[,i_vorm,i_ho]  > 0 &
                     p_HUAL_adj[,i_vorm,i_ho] < 0.05, 3,               
              ifelse(diff_HUAL[,i_vorm,i_ho]  < 0 & 
                     p_HUAL_adj[,i_vorm,i_ho] < 0.05, 2, 1))
plot(diff_HUAL[,i_vorm,i_ho], col = groepering, pch = 19, 
     xlab = "NSE vraag", 
...
for (i_ho in 1:NHO)
{
  for (i_vorm in 1:Nvorm)
  {
    tempR6 <- data.frame(Opleidingsvorm3 = NSEvormen[i_vorm], 
                         ...
                         p               = p_HUR6_adj[,i_vorm,i_ho], 
                         Niveau          = rep("HU",Nvragen),
                         HUgem           = gem_HU[,i_vorm,i_ho],
                         Benchmarkgem    = gem_R6[,i_vorm,i_ho],
                         gem_delta       = diff_HUR6[,i_vorm,i_ho],
                         N_HU            = N_HU[,i_vorm,i_ho],
                         N_benchmark     = N_R6[,i_vorm,i_ho],
    ...
    tempAL <- data.frame(Opleidingsvorm3 = NSEvormen[i_vorm],
                         ...
                         p               = p_HUAL_adj[,i_vorm,i_ho],
                         Niveau          = rep("HU",Nvragen),
                         HUgem           = gem_HU[,i_vorm,i_ho],
                         Benchmarkgem    = gem_AL[,i_vorm,i_ho],
                         gem_delta       = diff_HUAL[,i_vorm,i_ho],
                         N_HU            = N_HU[,i_vorm,i_ho],
                         N_benchmark     = N_AL[,i_vorm,i_ho],
                         Afnamejaar      = rep(2021,Nvragen) )
