...
```{r Inlezen .csv}
NSEjaar = 2026 # Kies het NSE jaar om te analyseren
...
psych::describeBy(nse_benchmark_clean[,vec_NSEvragen], vec_groepHU)
psych::describeBy(nse_benchmark_clean[,vec_NSEvragen], vec_groepR6)
psych::describeBy(nse_benchmark_clean[,vec_NSEvragen], vec_groepAL)
...
#Starttijd opslaan voor tijdsinschatting
t_start <- Sys.time()
...
for (i_croho in 1:Ncrohos)
{
  ...
  #Voortgang tonen
  t_nu <- Sys.time()
  t_verstreken <- difftime(t_nu, t_start, units = "mins")
  if (i_croho > 1) {
    t_per_croho <- as.numeric(t_verstreken) / (i_croho - 1)
    t_resterend <- round(t_per_croho * (Ncrohos - i_croho + 1), 1)
    cat(sprintf("\r[%s] Croho %d/%d (%.0f%%) | Verstreken: %.1f min | Geschat resterend: %.1f min",
                format(t_nu, "%H:%M:%S"), i_croho, Ncrohos,
                (i_croho - 1) / Ncrohos * 100, as.numeric(t_verstreken), t_resterend))
  } else {
    cat(sprintf("\r[%s] Croho %d/%d (0%%) | Gestart...",
                format(t_nu, "%H:%M:%S"), i_croho, Ncrohos))
  }
  flush.console()
  ...
  for (i_ho in 1:NHO)
  {
    ...
    for (i_vorm in 1:int_vorm)
    {
      ...
      for (i_vraag in vec_NSEvragen)
      {
        ...
        #Groepsgroote HU-groep moet minimaal 5 responses zijn
        ...
        {
          #Groepsgrootte R6 benchmark moet minimaal 5 responses groot zijn om te kunnen vergelijken
          ...
          #Groepsgrootte AL benchmark moet minimaal 5 responses groot zijn om te kunnen vergelijken
          ...
          arr_p_HUAL_w[i_vraag-Ncor,i_vorm,i_ho,i_croho] <- unname(wtd.t.test(data_HU,
                                                                          ...
                                                                          bootse   = TRUE)
                                                               $coefficients[3])
          arr_p_HUAL[i_vraag-Ncor,i_vorm,i_ho,i_croho]  <- tryCatch(
            t.test(data_HU, data_AL)$p.value, error = function(e) NA)
...
cat(sprintf("\nKlaar! Totale looptijd: %.1f minuten\n", as.numeric(difftime(Sys.time(), t_start, units = "mins"))))
...
write.csv2(nse_significanties, file = paste("C:/Users/",user,"/Stichting Hogeschool Utrecht/FCA-DA-P - Analytics/NSE/data/NSEsignificantietabel", NSEjaar, "2.csv", sep = ""))
```
