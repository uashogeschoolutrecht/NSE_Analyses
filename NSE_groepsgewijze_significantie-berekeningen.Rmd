...
Nvarianten <- length(varianttypes)
```
...
p_HUR5_variant <- matrix(,Nvragen, Nvarianten)
...
{
  i_vraag <- 32
    p_HUR5_variant[i_vraag-Ncor,1] <- t.test(HUscores[HUscores$Opleidingsvorm3 ==  varianttypes[1],i_vraag], R5scores[R5scores$Opleidingsvorm3 == varianttypes[1],i_vraag])$p.value
}

#Voor elke NSEvraag de respons tussen groepen vergelijken en p waarde opslaan
for (i_vraag in vec_NSEvragen)
  {
  p_HUR5[i_vraag-Ncor]   <- t.test(HUscores[,i_vraag], 
                                   R5scores[,i_vraag])$p.value
  p_HUAlle[i_vraag-Ncor] <- t.test(HUscores[,i_vraag], 
                                   Allescores[,i_vraag])$p.value
  gem_HU[i_vraag-Ncor]   <- t.test(HUscores[,i_vraag], 
                                   R5scores[,i_vraag])$estimate[1]
