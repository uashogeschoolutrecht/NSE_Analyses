...
instituten <- c("Totaal", "Instituut Archimedes")

#Weergeven van de verschillende groepen
groepHU; groepR6; vormtypes; HOtypes; crohos; instituten
```
...
#Maak voor elk instituut een logische vector en sla op als lijst
vec_instituut  <- vector(mode = "list", 2)
vec_instituut[[1]] <- nse_benchmark_clean$CrohoNaamActueel %in% crohos[c(43:56,80:88)]
vec_instituut[[2]] <- rep(TRUE, nrow(nse_benchmark_clean))
N_instituut <- length(vec_instituut)
NSEinstituten  <- c("Instituut Archimedes", "Totaal")
...
p_HUR6      <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
p_HUR6_w    <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
p_HUAL      <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
p_HUAL_w    <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
N_HU        <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
N_R6        <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
N_AL        <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
gem_HU      <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
gem_R6      <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
gem_AL      <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
diff_HUR6   <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
diff_HUAL   <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))

#Definieer correctiefactor zodat index voor het vullen van p waardes op 1 begint en niet op 28
Ncor <- Neerste - 1 

#Voor elk instituut
for (i_instituut in 1:N_instituut)
{
    ...
    for (i_ho in 1:NHO)
    {
      ...
      for (i_vorm in 1:Nvorm)
      {
        ...
        for (i_vraag in vec_NSEvragen)
        {
          ...
          N_HU[i_vraag-Ncor, i_vorm, i_ho, i_croho, i_instituut] <- sum(!is.na(data_HU))
          N_R6[i_vraag-Ncor, i_vorm, i_ho, i_croho, i_instituut] <- sum(!is.na(data_R6))
          N_AL[i_vraag-Ncor, i_vorm, i_ho, i_croho, i_instituut] <- sum(!is.na(data_AL))
          
          #Berekenen gemiddelden per groep (gewogen)
          gem_HU[i_vraag-Ncor,i_vorm,i_ho, i_croho, i_instituut]  <- weighted.mean(data_HU,
                                                                                   data_w_HU,
                                                                                   na.rm = TRUE)
          gem_R6[i_vraag-Ncor,i_vorm,i_ho, i_croho, i_instituut]  <- weighted.mean(data_R6,
                                                                                   data_w_R6,
                                                                                   na.rm = TRUE)
          gem_AL[i_vraag-Ncor,i_vorm,i_ho, i_croho, i_instituut]  <- weighted.mean(data_AL,
                                                                                   data_w_AL,
                                                                                   na.rm = TRUE)
          
          #Berekenen verschil tussen gemiddelden (delta)
          diff_HUR6[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut] <-gem_HU[i_vraag-Ncor,
                                                                            i_vorm,
                                                                            i_ho,
                                                                            i_croho, i_instituut]-gem_R6[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]
          diff_HUAL[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut] <-gem_HU[i_vraag-Ncor,
                                                                            i_vorm,
                                                                            i_ho,
                                                                            i_croho, i_instituut]-gem_AL[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]
          
          #Groepsgroote moet minimaal 5 responses zijn en vraag 64 wordt overgeslagen
          if (N_HU[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]< 5 |
              N_R6[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]< 5 |
              N_AL[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]< 5 |
              i_vraag == i_exclusie)
          {
          ...
          {
            #Berekenen p waarden
            p_HUR6_w[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut] <- unname(wtd.t.test(data_HU,
                                                                                         data_R6, 
                                                                                         weight   = data_w_HU,
                                                                                         weighty  = data_w_R6,
                                                                                         mean1    = TRUE, 
            ...
            p_HUAL_w[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut] <- unname(wtd.t.test(data_HU,
                                                                                         data_AL, 
                                                                                         weight   = data_w_HU,
                                                                                         weighty  = data_w_AL,
                                                                                         mean1    = TRUE, 
            ...
            p_HUR6[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]  <- t.test(data_HU
                                                                             ,data_R6
            )$p.value
            p_HUAL[i_vraag-Ncor,i_vorm,i_ho,i_croho, i_instituut]  <- t.test(data_HU
                                                                             ,data_AL
            )$p.value
...
```{r FDR correctie}
p_HUR6_adj  <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
p_HUAL_adj  <- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
p_HUR6_w_adj<- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))
p_HUAL_w_adj<- array(NA, c(Nvragen, Nvorm, NHO, Ncrohos, N_instituut))

#Loop over alle instituten
for (i_instituut in 1:N_instituut)
{
  ...
  for (i_croho in 1:Ncrohos)
  {
    ...
    for (i_ho in 1:NHO)
    {
      ...
      for (i_vorm in 1:Nvorm)
      {
        p_HUR6_adj[,i_vorm,i_ho,i_croho,i_instituut]   <- p.adjust(p_HUR6[,i_vorm
                                                                          ,i_ho
                                                                          ,i_croho
                                                                          ,i_instituut]
                                                                   ,method = "BY")
        p_HUAL_adj[,i_vorm,i_ho,i_croho,i_instituut]   <- p.adjust(p_HUAL[,i_vorm
                                                                          ,i_ho,i_croho
                                                                          ,i_instituut]
                                                                   ,method = 'BY')
        p_HUR6_w_adj[,i_vorm,i_ho,i_croho,i_instituut] <- p.adjust(p_HUR6_w[,i_vorm
                                                                            ,i_ho
                                                                            ,i_croho
                                                                            ,i_instituut]
                                                                   ,method = "BY")
        p_HUAL_w_adj[,i_vorm,i_ho,i_croho,i_instituut] <- p.adjust(p_HUAL_w[,i_vorm
                                                                            ,i_ho
                                                                            ,i_croho
                                                                            ,i_instituut]
                                                                   ,method = 'BY')
...
i_vorm = 4; i_ho = 4; i_croho = 4; i_instituut = 2
plot(p_HUR6[,i_vorm,i_ho,i_croho,i_instituut], p_HUR6_adj[,i_vorm,i_ho,i_croho,i_instituut]); abline(h = 0.05, v=0.05)
rm(p_HUR6, p_HUAL)
```

```{r Visualiseren test-statistiek HU <> R6}
#Bekijken uitkomstmaten (delta inclusief significantie in kleur)
plot(p_HUR6_adj[,i_vorm,i_ho,i_croho,i_instituut])
abline(h = 0.05)
groepering <- ifelse(diff_HUR6[,i_vorm,i_ho,i_croho,i_instituut]  > 0 & 
                     p_HUR6_adj[,i_vorm,i_ho,i_croho,i_instituut] < 0.05, 3, 
              ifelse(diff_HUR6[,i_vorm,i_ho,i_croho,i_instituut]  < 0 & 
                     p_HUR6_adj[,i_vorm,i_ho,i_croho,i_instituut] < 0.05, 2, 1))
plot(diff_HUR6[,i_vorm,i_ho,i_croho,i_instituut], col = groepering, pch = 19, 
     xlab = "NSE vraag", 
     ylab = "Gemiddelde verschil (delta) tussen HU en R6")
```

```{r Visualiseren test-statistiek HU <> AL}
plot(p_HUAL_adj[,i_vorm,i_ho,i_croho,i_instituut])
abline(h = 0.05)
groepering <- ifelse(diff_HUAL[,i_vorm,i_ho,i_croho,i_instituut]  > 0 &
                     p_HUAL_adj[,i_vorm,i_ho,i_croho,i_instituut] < 0.05, 3,               
              ifelse(diff_HUAL[,i_vorm,i_ho,i_croho,i_instituut]  < 0 & 
                     p_HUAL_adj[,i_vorm,i_ho,i_croho,i_instituut] < 0.05, 2, 1))
plot(diff_HUAL[,i_vorm,i_ho,i_croho,i_instituut], col = groepering, pch = 19, 
     xlab = "NSE vraag", 
    ...
    for (i_ho in 1:NHO)
    {
      for (i_vorm in 1:Nvorm)
      {
        tempR6 <- data.frame(Opleidingsvorm3 = NSEvormen[i_vorm], 
                             Instituut       = NSEinstituten[i_instituut],
                             CrohoActueel    = NSEcrohos[i_croho], 
                             BaMa            = NSEHOtypes[i_ho],
                             Vraag           = NSEvragen, 
                             Benchmark       = rep("R6",Nvragen),  
                             p               = p_HUR6_adj[,i_vorm,i_ho,i_croho,i_instituut], 
                             p_w             = p_HUR6_w_adj[,i_vorm,i_ho,i_croho,i_instituut],
                             HUgem           = gem_HU[,i_vorm,i_ho,i_croho,i_instituut],
                             Benchmarkgem    = gem_R6[,i_vorm,i_ho,i_croho,i_instituut],
                             gem_delta       = diff_HUR6[,i_vorm,i_ho,i_croho,i_instituut],
                             N_HU            = N_HU[,i_vorm,i_ho,i_croho,i_instituut],
                             N_benchmark     = N_R6[,i_vorm,i_ho,i_croho,i_instituut],
        ...
        tempAL <- data.frame(Opleidingsvorm3 = NSEvormen[i_vorm],
                             ...
                             p               = p_HUAL_adj[,i_vorm,i_ho,i_croho,i_instituut],
                             p_w             = p_HUAL_w_adj[,i_vorm,i_ho,i_croho,i_instituut],
                             HUgem           = gem_HU[,i_vorm,i_ho,i_croho,i_instituut],
                             Benchmarkgem    = gem_AL[,i_vorm,i_ho,i_croho,i_instituut],
                             gem_delta       = diff_HUAL[,i_vorm,i_ho,i_croho,i_instituut],
                             N_HU            = N_HU[,i_vorm,i_ho,i_croho,i_instituut],
                             N_benchmark     = N_AL[,i_vorm,i_ho,i_croho,i_instituut],
                             Afnamejaar      = rep(2021,Nvragen) )
