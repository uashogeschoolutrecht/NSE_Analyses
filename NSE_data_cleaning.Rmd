...
sub_nse_benchmark <- nse_benchmark[nse_benchmark$SoortHo == 1,] #SoortHo = 1 filtert alle uni's weg
unique(sub_nse_benchmark$BrinNaamActueel)
```
```{r Missende responses (-1 t/m -9) naar NA}
vec_missingdata = c(-1:-9) #Missende data wordt aangeven met codes -1 t/m -9
nse_benchmark_clean <- sub_nse_benchmark #Gecleande data is gebaseerd op originele data
...
rm(nse_benchmark, sub_nse_benchmark)
```

# 5. Opslaan
```{r Save dataframe en 2 vectoren}
save(nse_benchmark_clean, str_NSEvragen, vec_NSEvragen, ind_exclusie, file = "~/research-drive/M21033303_DenA (Projectfolder)/DA_Onderzoek/2021nse/data_in/NSE2021 nse_benchmark_clean.RData")
```