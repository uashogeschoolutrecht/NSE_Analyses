...
nse_benchmark <- read.csv2(paste("C:/Users/",user,"/Stichting Hogeschool Utrecht/FCA-DA-P - Analytics/NSE/data/nse", NSEjaar,"_cleaned.csv",sep="", dec ="."))
...
nse_benchmark$BEGELEIDING_SELECTIE <- NULL #Kolom verwijderen uit dataset
nse_benchmark$Weegfactor <- as.numeric(nse_benchmark$Weegfactor) #Weegfactor naar numeriek converteren
...
# Vind de R6 instellingsnamen op basis van BRIN
# R6 = Hogeschool van Amsterdam, De Haagse Hogeschool, Hogeschool Inholland, Hogeschool Leiden en Hogeschool Rotterdam. 
codes <- c("28DN", "27UM", "27PZ", "21RI","22OJ")

# Functie die op basis van de BRIN de eerste instellingsnaam vindt in de data
groepR6 <- unname(sapply(codes, function(code) {
  match_index <- which(nse_benchmark_clean$BrinActueel == code)[1]
  if (!is.na(match_index)) {
    return(nse_benchmark_clean$BrinNaam[match_index])
  } else {
    return(NA)
  }
}))
